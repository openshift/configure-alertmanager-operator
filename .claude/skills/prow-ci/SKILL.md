...
```
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_configure_alertmanager_operator/<PR_NUMBER>/pull-ci-openshift-configure-alertmanager-operator-master-lint/<BUILD_ID>
```
...
**Job History**: https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator
...
```bash
# View recent PR jobs
curl -s "https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator&type=presubmit" | grep -E "pull-ci-openshift-configure-alertmanager-operator"
...
- **Pull request jobs**: `gs://test-platform-results/pr-logs/pull/openshift_configure_alertmanager_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]`
- **Periodic jobs**: `gs://test-platform-results/logs/[JOB_NAME]/[JOB_ID]`

**Viewing logs via web**:
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_configure_alertmanager_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]
```
...
- `pull-ci-openshift-configure-alertmanager-operator-master-e2e-binary-build-success` - E2E binary build verification
- `pull-ci-openshift-configure-alertmanager-operator-master-coverage` - Code coverage analysis (with Codecov)
- `pull-ci-openshift-configure-alertmanager-operator-master-lint` - Linting checks
- `pull-ci-openshift-configure-alertmanager-operator-master-test` - Unit tests
- `pull-ci-openshift-configure-alertmanager-operator-master-validate` - Validation checks

**Tekton Pipelines** (configured in `.tekton/`):
- `configure-alertmanager-operator-pull-request` - Main PR pipeline (docker build with OCI-TA)
- `configure-alertmanager-operator-e2e-pull-request` - E2E testing pipeline
- `configure-alertmanager-operator-pko-pull-request` - PKO (Package Operator) pipeline
...
```text
https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator&type=presubmit
```
...
- Configuration: `ci-operator/config/openshift/configure-alertmanager-operator/openshift-configure-alertmanager-operator-master.yaml`
- Runs: lint, test, validate, coverage, e2e-binary-build
- Uses Codecov for coverage reporting (secret: `configure-alertmanager-operator-codecov-token`)
...
- Builds container images to Quay (configure-alertmanager-operator-tenant)
...
# Option 2: Use gh CLI to find URLs
gh pr checks <PR_NUMBER> | grep "pull-ci-openshift-configure-alertmanager-operator"
...
# https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator

# Or use platform-specific command:
# macOS: open "https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator"
# Linux: xdg-open "https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator"
# Windows: start "https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator"

# View specific PR on Prow (replace <PR_NUMBER>)
# https://prow.ci.openshift.org/?repo=openshift%2Fconfigure-alertmanager-operator&type=presubmit&pull=<PR_NUMBER>
```
...
- Prow jobs: `pull-ci-openshift-configure-alertmanager-operator-master-*`
- Tekton jobs: Usually show as "Tekton" or pipeline names in PR checks
- Verify repo name format in Prow: `openshift_configure_alertmanager_operator` (underscore, not dash)
...
- Common issues:
  ...
  - Auth failures → Secret configuration in `configure-alertmanager-operator-tenant` namespace
- Local validation:
  ```bash
...
- Location: `ci-operator/config/openshift/configure-alertmanager-operator/openshift-configure-alertmanager-operator-master.yaml`
- Update process: Submit PR to openshift/release repository
- Auto-generated jobs in: `ci-operator/jobs/openshift/configure-alertmanager-operator/`

**Tekton Pipelines** (in this repo):
- Location: `.tekton/` directory
- Files:
  - `configure-alertmanager-operator-pull-request.yaml` - Main PR pipeline
  - `configure-alertmanager-operator-push.yaml` - Post-merge pipeline
  - `configure-alertmanager-operator-e2e-pull-request.yaml` - E2E testing
  - `configure-alertmanager-operator-pko-pull-request.yaml` - PKO validation
...
- Secret: `configure-alertmanager-operator-codecov-token` (stored in Prow)
- Generate coverage locally: `make coverage`
- Coverage runs on PRs and post-merge (`publish-coverage`)
- Dashboard: Check Codecov for configure-alertmanager-operator
